...
const redrawKeyboard = () => {
  ...
  (capsLock) ? keys.namedItem("key-CL").style.opacity = 0.5 : keys.namedItem("key-CL").style.opacity = 1;
  (shift) ? keys.namedItem("key-SHIFT").style.opacity = 0.5 : keys.namedItem("key-SHIFT").style.opacity = 1;
  for (let i = 0; i < keys.length; i++) {
    if (keys[i].id.length === 5 && !Number.isInteger(keys[i].id[4]))
      keys[i].innerHTML = (shift || capsLock) ? keys[i].id[4].toUpperCase() : keys[i].id[4];
  }
}